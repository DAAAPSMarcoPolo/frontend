...
const BacktestList = ({ backtests, backtestSelected, selectBacktest }) => (
    <div className="backtest margins">
        <ul className="nav-tabs nav-overflow scroll-hide">
            {backtests && backtests.map((backtest, i) => <li className={`tab select-backtest ${backtestSelected.backtest.id === backtest.backtest.id && 'active'}`} key={i} onClick={() => selectBacktest(i, backtest.backtest.id)}> {backtest.backtest.id}</li> )}
        </ul>
